...
def load_questions(file_path: str, limit: int = NUM_QUESTIONS) -> List[Dict[str, Any]]:
    """
...
def run_direct_qa_baseline(
    ...
    for item in questions:
        ...
        response = debater.direct_answer(
            question_text,
            temperature=TEMPERATURE_DIRECT
        )
...
def run_self_consistency_baseline(
    questions: List[Dict[str, Any]],
    model_name: str = MODEL_NAME,
    num_samples: int = SELF_CONSISTENCY_SAMPLES
) -> List[Dict[str, Any]]:
    """
...
def run_debate_pipeline(
    ...
    for item in questions:
        ...
        results.append({
            ...
            "num_rounds_used": debate_result.get("num_rounds_used", 0)
        })
...
def main() -> None:
    ...
    # -----------------------------
    if not os.getenv("MY_API_KEY1"):
        print("Warning: MY_API_KEY1 is not set.")
